...
static keypad_sw_t      _keypad_sw_list[KEYPAD_SW_ID_MAX - KEYPAD_SW_ID_MIN] = {
...
static __attribute__((noreturn)) THD_FUNCTION(_keypad_poll_thread, arg)
{
  ...
  while(true)
  {
      ...
      for(sw_id = 0; sw_id < (KEYPAD_SW_ID_MAX - KEYPAD_SW_ID_MIN); sw_id++)
      {
 ...
 * Shell functions
 */
void keypad_loop_key(BaseSequentialStream *chp, int argc, char *argv[])
{
  (void)argv;
  if (argc > 0) {
    chprintf(chp, "Usage: kp-loop-keys\r\n");
    return;
  }
